...
def upgrade():
    ...
    if is_postgres:
        ...
        with op.batch_alter_table("users") as batch_op:
            batch_op.alter_column(
                "password_hash", existing_type=sa.String(length=200), type_=sa.String(length=255), nullable=True
            )
    ...
    if is_postgres:
        ...
        op.execute(
            "COMMENT ON COLUMN system_configs.category IS '配置分类：general=通用 auth=认证 security=安全 email=邮件 storage=存储'"
        )
        op.execute(
            "COMMENT ON COLUMN system_configs.value_type IS '值类型：string=字符串 int=整数 float=浮点数 bool=布尔 json=JSON对象'"
        )
...
def downgrade():
    ...
    op.alter_column(
        "users", "password_hash", existing_type=sa.String(length=255), type_=sa.String(length=200), nullable=True
    )
