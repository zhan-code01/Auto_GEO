...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column("task_id", sa.Integer(),
                  sa.ForeignKey("auto_publish_tasks.id", ondelete="SET NULL"), nullable=True),
        sa.Column("record_id", sa.Integer(),
                  sa.ForeignKey("auto_publish_records.id", ondelete="SET NULL"), nullable=True),
        sa.Column("device_id", sa.String(length=64), nullable=True),
        sa.Column("checkpoint", sa.String(length=30), nullable=False,
    ...
    op.create_table(
        ...
        sa.Column("tier", sa.String(length=20), server_default="free",
                  comment="套餐级别: free/pro/enterprise"),
        ...
        sa.Column("platform_whitelist", sa.JSON(), nullable=True,
                  comment="允许发布的平台列表(空=全部允许)"),
        sa.Column("created_at", sa.DateTime(), server_default=sa.func.now()),
