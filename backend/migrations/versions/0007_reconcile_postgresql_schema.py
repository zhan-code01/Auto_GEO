...
def _index_exists(index_name):
    ...
    if bind.dialect.name != "postgresql":
        return False
    result = bind.execute(sa.text("SELECT 1 FROM pg_indexes WHERE indexname = :name"), {"name": index_name})
...
def upgrade():
    ...
    if not _table_exists("knowledge_categories"):
        ...
        _create_index_if_not_exists(
            "ix_kc_ragflow_dataset_id", "knowledge_categories", ["ragflow_dataset_id"], unique=True
        )
    ...
    if not _table_exists("knowledge_items"):
        ...
        _create_index_if_not_exists(
            "ix_ki_ragflow_document_id", "knowledge_items", ["ragflow_document_id"], unique=True
        )
    ...
    if not _table_exists("user_agent_preferences"):
        op.create_table(
            ...
            sa.Column(
                "onboarding_dismissed", sa.Boolean(), server_default="0", nullable=False, comment="是否已关闭新用户引导"
            ),
            sa.Column("created_at", sa.DateTime(), server_default=sa.func.now()),
            sa.Column("updated_at", sa.DateTime(), server_default=sa.func.now()),
            sa.ForeignKeyConstraint(["system_user_id"], ["users.id"], ondelete="CASCADE"),
    ...
    if _table_exists("clients"):
        _add_column_if_not_exists(
            "clients", "user_id", sa.Column("user_id", sa.Integer(), nullable=True, comment="所属用户ID")
        )
        if _column_exists("clients", "user_id"):
            ...
            try:
                op.create_foreign_key(
                    "fk_clients_user_id", "clients", "users", ["user_id"], ["id"], ondelete="SET NULL"
                )
            except Exception:
                pass  # 外键已存在
    ...
    if _table_exists("auto_publish_tasks"):
        _add_column_if_not_exists(
            "auto_publish_tasks", "user_id", sa.Column("user_id", sa.Integer(), nullable=True, comment="数据归属用户ID")
        )
        if _column_exists("auto_publish_tasks", "user_id"):
            _create_index_if_not_exists("ix_apt_user_id", "auto_publish_tasks", ["user_id"])
            try:
                op.create_foreign_key(
                    "fk_apt_user_id", "auto_publish_tasks", "users", ["user_id"], ["id"], ondelete="SET NULL"
                )
            except Exception:
                pass
    ...
    if _table_exists("conversation_sessions"):
        _add_column_if_not_exists(
            "conversation_sessions", "title", sa.Column("title", sa.String(120), nullable=True, comment="会话标题")
        )
    ...
    if _table_exists("accounts"):
        _add_column_if_not_exists(
            "accounts", "username", sa.Column("username", sa.String(100), nullable=True, comment="平台内的用户名")
        )
