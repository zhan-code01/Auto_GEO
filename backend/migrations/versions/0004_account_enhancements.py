...
def upgrade():
    ...
    op.create_foreign_key(
        "fk_accounts_group_id", "accounts", "account_groups", ["group_id"], ["id"], ondelete="SET NULL"
    )
    ...
    # 健康度
    op.add_column(
        "accounts", sa.Column("health_score", sa.Integer(), nullable=True, server_default="100", comment="健康度评分")
    )
    ...
    op.add_column(
        "accounts",
        sa.Column("browser_type", sa.String(20), nullable=True, server_default="playwright", comment="浏览器类型"),
    )
    op.add_column(
        "accounts", sa.Column("adspower_profile_id", sa.String(100), nullable=True, comment="AdsPower Profile ID")
    )
