...
def test_ragflow_client_initialization():
    """测试RAGFlowClient初始化"""
    log_print("测试1: RAGFlowClient初始化...")
    try:
...
def test_geo_ragflow_integration_initialization():
    ...
    log_print("测试2: GeoRAGFlowIntegration初始化...")
    try:
...
def test_url_construction():
    ...
    try:
        ...
        log_print(f"  ✗ URL构建失败: {e}")
        return False


def test_config_file():
    """测试配置文件格式"""
    log_print("测试5: 配置文件格式...")
    try:
...
def test_dependencies():
    ...
    try:
        ...
        log_print(f"  ✓ requests版本: {requests.__version__}")
        return True
    except ImportError as e:
        log_print(f"  ✗ 依赖项导入失败: {e}")
        return False


def run_mock_tests():
    """运行模拟测试（模拟API调用）"""
    log_print("测试7: 模拟API调用...")
    try:
        # 模拟API响应
...
def test_integration_methods():
    ...
    try:
        ...
        log_print(f"  ✗ 集成类方法测试失败: {e}")
        return False


def main():
    """运行所有测试"""
    log_print("开始测试RAGFlow与geo项目集成框架...")
    log_print("=" * 50)
    ...
    log_print("=" * 50)
    log_print(f"测试完成! 通过: {sum(results)}/8")

    if all(results):
        log_print("🎉 所有测试通过! 集成框架可以正常运行。")
        log_print("\n要进行实际连接测试，请:")
        log_print("1. 确保RAGFlow服务正在运行")
        log_print("2. 更新config_example.json中的配置信息")
        log_print("3. 运行: python test_connection.py")
    else:
        log_print("⚠️  部分测试失败，请检查代码和依赖项。")
