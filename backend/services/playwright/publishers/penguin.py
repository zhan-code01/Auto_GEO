...
企鹅号平台: https://om.qq.com/article/articlePublish
企鹅号是腾讯旗下的内容创作平台。
"""
...
class PenguinPublisher(BasePublisher):
    """企鹅号文章发布器"""
    ...
    async def publish(self, page: Page, article: Any, account: Any, declare_ai_content: bool = True) -> Dict[str, Any]:
        ...
        try:
            logger.info("🚀 [企鹅号] 开始发布文章...")

            rate = self._check_rate_limit()
            if not rate["allowed"]:
                logger.warning(f"⚠️ [企鹅号] 频率限制: {rate['reason']}")
            ...
            if len(content) > self.MAX_CONTENT_LENGTH:
                logger.warning(f"⚠️ [企鹅号] 正文{len(content)}字，超过限制{self.MAX_CONTENT_LENGTH}字，将截断")
                content = content[:self.MAX_CONTENT_LENGTH]

            stage = "navigate"
            publish_url = self.config.get("publish_url", "https://om.qq.com/article/articlePublish")
            ...
            logger.info(f"📷 [企鹅号] 已准备 {len(image_paths)} 张图片")
            ...
            logger.exception(f"❌ [企鹅号] 发布失败 stage={stage}: {e}")
    ...
    async def _ensure_logged_in(self, page: Page) -> None:
        ...
        if "login" in current_url or "signin" in current_url:
            raise RuntimeError("企鹅号账号未登录，请重新授权")
        ...
        for selector in title_selectors:
            try:
                ...
                if await inp.count() > 0 and await inp.is_visible(timeout=3000):
                    ...
                    logger.info(f"✅ [企鹅号] 标题已填充: {title[:30]}...")
                    return True
            except Exception as e:
                logger.debug(f"[企鹅号] 标题选择器 {selector} 失败: {e}")
                continue
        return False
        ...
        for selector in content_selectors:
            try:
                ...
                if await editor.count() > 0 and await editor.is_visible(timeout=3000):
                    ...
                    logger.info(f"✅ [企鹅号] 正文已填充: {len(plain_content)} 字符")
                ...
                logger.debug(f"[企鹅号] 内容选择器 {selector} 失败: {e}")
                continue
    ...
    async def _upload_images(self, page: Page, image_paths: List[str]) -> bool:
        ...
        try:
            ...
            for i, img_path in enumerate(image_paths[:20]):
                logger.info(f"📷 [企鹅号] 上传第 {i+1}/{len(image_paths)} 张图片...")
                ...
                for selector in upload_selectors:
                    try:
                        ...
                        if await file_input.count() > 0 and await file_input.is_visible(timeout=2000):
                            ...
                            uploaded = True
                            logger.success(f"✅ [企鹅号] 第 {i+1} 张图片上传成功")
                            await asyncio.sleep(2)
                            break
                    except Exception as e:
                        logger.debug(f"[企鹅号] 上传选择器 {selector} 失败: {e}")
                        continue

                if not uploaded:
                    try:
                        await self._paste_image_via_clipboard(page, img_path)
                        logger.success(f"✅ [企鹅号] 第 {i+1} 张图片粘贴成功")
                        await asyncio.sleep(2)
                    except Exception as e:
                        logger.warning(f"⚠️ [企鹅号] 第 {i+1} 张图片上传失败: {e}")

        except Exception as e:
            logger.warning(f"⚠️ [企鹅号] 图片上传流程异常: {e}")
    ...
    async def _paste_image_via_clipboard(self, page: Page, image_path: str) -> bool:
        ...
        try:
            ...
            logger.warning(f"[企鹅号] 剪贴板粘贴图片失败: {e}")
            return False
        ...
        for selector in publish_selectors:
            try:
                ...
                if await btn.count() > 0 and await btn.is_visible(timeout=3000):
                    await btn.click()
                    logger.info("✅ [企鹅号] 已点击发布按钮")
                    await asyncio.sleep(2)
                    return True
            except Exception as e:
                logger.debug(f"[企鹅号] 发布按钮 {selector} 失败: {e}")
                continue
    ...
    async def _wait_for_result(self, page: Page) -> Dict[str, Any]:
        ...
        for i in range(60):  # 60次 × 2秒 = 120秒
            ...
            logger.debug(f"[企鹅号] 第{(i+1)*2}秒, URL: {current_url}")
        ...
        logger.error(f"❌ [企鹅号] 发布超时（已等待120秒）: {page.url}")
...
registry.register("penguin", PenguinPublisher("penguin", PENGUIN_CONFIG))
