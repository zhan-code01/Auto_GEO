...
网易号平台: https://mp.163.com/admin/article/publish
网易号是网易旗下的内容创作平台。
"""
...
class WangyiPublisher(BasePublisher):
    """网易号文章发布器"""
    ...
    async def publish(self, page: Page, article: Any, account: Any, declare_ai_content: bool = True) -> Dict[str, Any]:
        ...
        try:
            logger.info("🚀 [网易号] 开始发布文章...")

            rate = self._check_rate_limit()
            if not rate["allowed"]:
                logger.warning(f"⚠️ [网易号] 频率限制: {rate['reason']}")
            ...
            if len(content) > self.MAX_CONTENT_LENGTH:
                logger.warning(f"⚠️ [网易号] 正文{len(content)}字，超过限制{self.MAX_CONTENT_LENGTH}字，将截断")
                content = content[:self.MAX_CONTENT_LENGTH]

            stage = "navigate"
            publish_url = self.config.get("publish_url", "https://mp.163.com/admin/article/publish")
            ...
            image_paths, image_temp_files = await materialize_images(article, limit=20)
            temp_files.extend(image_temp_files)
            logger.info(f"📷 [网易号] 已准备 {len(image_paths)} 张图片")
            ...
            logger.exception(f"❌ [网易号] 发布失败 stage={stage}: {e}")
    ...
    async def _ensure_logged_in(self, page: Page) -> None:
        ...
        if "login" in current_url or "signin" in current_url:
            raise RuntimeError("网易号账号未登录，请重新授权")
    ...
    async def _fill_title(self, page: Page, title: str) -> bool:
        """填充标题"""
        title_selectors = ['input[placeholder*="标题"]', 'input[name*="title"]', 'input.article-title', '#title']
        for selector in title_selectors:
            try:
                ...
                if await inp.count() > 0 and await inp.is_visible(timeout=3000):
                    ...
                    logger.info(f"✅ [网易号] 标题已填充: {title[:30]}...")
                    return True
            except Exception as e:
                logger.debug(f"[网易号] 标题选择器 {selector} 失败: {e}")
                continue
        return False
            ...
            try:
                ...
                if await editor.count() > 0 and await editor.is_visible(timeout=3000):
                    ...
                    await page.evaluate(
                        ...
                        document.querySelector('textarea[name*="content"], .ql-editor, [contenteditable="true"]')?.dispatchEvent(ev);
                    }""",
                        plain_content,
                    )
                    logger.info(f"✅ [网易号] 正文已填充: {len(plain_content)} 字符")
                ...
                logger.debug(f"[网易号] 内容选择器 {selector} 失败: {e}")
                continue
    ...
    async def _upload_images(self, page: Page, image_paths: List[str]) -> bool:
        ...
        try:
            ...
            for i, img_path in enumerate(image_paths[:20]):
                logger.info(f"📷 [网易号] 上传第 {i+1}/{len(image_paths)} 张图片...")
                ...
                for selector in upload_selectors:
                    try:
                        ...
                        if await file_input.count() > 0 and await file_input.is_visible(timeout=2000):
                            ...
                            uploaded = True
                            logger.success(f"✅ [网易号] 第 {i+1} 张图片上传成功")
                            await asyncio.sleep(2)
                            break
                    except Exception as e:
                        logger.debug(f"[网易号] 上传选择器 {selector} 失败: {e}")
                        continue

                if not uploaded:
                    try:
                        await self._paste_image_via_clipboard(page, img_path)
                        logger.success(f"✅ [网易号] 第 {i+1} 张图片粘贴成功")
                        await asyncio.sleep(2)
                    except Exception as e:
                        logger.warning(f"⚠️ [网易号] 第 {i+1} 张图片上传失败: {e}")

        except Exception as e:
            logger.warning(f"⚠️ [网易号] 图片上传流程异常: {e}")
    ...
    async def _paste_image_via_clipboard(self, page: Page, image_path: str) -> bool:
        ...
        try:
            ...
            await page.evaluate(
                ...
                const dt = new DataTransfer();
                dt.items.add(file);
                const target = document.querySelector('textarea[name*="content"], .ql-editor, [contenteditable="true"]');
                if (target) {
            ...
            logger.warning(f"[网易号] 剪贴板粘贴图片失败: {e}")
            return False
        ...
        for selector in publish_selectors:
            try:
                ...
                if await btn.count() > 0 and await btn.is_visible(timeout=3000):
                    await btn.click()
                    logger.info("✅ [网易号] 已点击发布按钮")
                    await asyncio.sleep(2)
                    return True
            except Exception as e:
                logger.debug(f"[网易号] 发布按钮 {selector} 失败: {e}")
                continue
    ...
    async def _wait_for_result(self, page: Page) -> Dict[str, Any]:
        ...
        for i in range(60):  # 60次 × 2秒 = 120秒
            ...
            logger.debug(f"[网易号] 第{(i+1)*2}秒, URL: {current_url}")

            try:
                success_selectors = ["text=发布成功", "text=已发布", "text=提交成功"]
                for sel in success_selectors:
                    elem = page.locator(sel).first
                    if await elem.count() > 0 and await elem.is_visible(timeout=500):
                        logger.success("🎉 [网易号] 检测到发布成功提示")
        ...
        logger.error(f"❌ [网易号] 发布超时（已等待120秒）: {page.url}")
...
registry.register("wangyi", WangyiPublisher("wangyi", WANGYI_CONFIG))
