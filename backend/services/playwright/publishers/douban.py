...
豆瓣平台: https://www.douban.com/note
豆瓣是一个集社交、评论、分享于一体的平台，也有日记/笔记功能。
"""
...
class DoubanPublisher(BasePublisher):
    """豆瓣日记发布器"""
    ...
    async def publish(self, page: Page, article: Any, account: Any, declare_ai_content: bool = True) -> Dict[str, Any]:
        ...
        try:
            logger.info("🚀 [豆瓣] 开始发布日记...")

            rate = self._check_rate_limit()
            if not rate["allowed"]:
                logger.warning(f"⚠️ [豆瓣] 频率限制: {rate['reason']}")
                return {"success": False, "error_msg": f"频率限制: {rate['reason']}"}

            title = getattr(article, "title", "") or "未命名日记"
            ...
            image_paths, image_temp_files = await materialize_images(article, limit=9)  # 豆瓣日记限制9张
            temp_files.extend(image_temp_files)
            logger.info(f"📷 [豆瓣] 已准备 {len(image_paths)} 张图片")
            ...
            logger.exception(f"❌ [豆瓣] 发布失败 stage={stage}: {e}")
    ...
    async def _ensure_logged_in(self, page: Page) -> None:
        ...
        if "login" in current_url or "signin" in current_url:
            raise RuntimeError("豆瓣账号未登录，请重新授权")

        login_markers = ["text=登录", "text=登录豆瓣", "text=立即登录"]
        ...
        for selector in title_selectors:
            try:
                ...
                if await inp.count() > 0 and await inp.is_visible(timeout=3000):
                    ...
                    logger.info(f"✅ [豆瓣] 标题已填充: {title[:30]}...")
                    return True
            except Exception as e:
                logger.debug(f"[豆瓣] 标题选择器 {selector} 失败: {e}")
                continue
        return False
            ...
            try:
                ...
                if await editor.count() > 0 and await editor.is_visible(timeout=3000):
                    ...
                    await page.evaluate(
                        ...
                        document.querySelector('textarea[name*="content"], #content, [contenteditable="true"]')?.dispatchEvent(ev);
                    }""",
                        plain_content,
                    )
                    logger.info(f"✅ [豆瓣] 正文已填充: {len(plain_content)} 字符")
                ...
                logger.debug(f"[豆瓣] 内容选择器 {selector} 失败: {e}")
                continue
            ...
            for i, img_path in enumerate(image_paths[:9]):  # 豆瓣限制9张
                ...
                for selector in upload_selectors:
                    try:
                        ...
                        logger.debug(f"[豆瓣] 上传选择器 {selector} 失败: {e}")
                        continue
    ...
    async def _paste_image_via_clipboard(self, page: Page, image_path: str) -> bool:
        ...
        try:
            ...
            await page.evaluate(
                ...
                const dt = new DataTransfer();
                dt.items.add(file);
                const target = document.querySelector('textarea[name*="content"], #content, [contenteditable="true"]');
                if (target) {
            ...
            logger.warning(f"[豆瓣] 剪贴板粘贴图片失败: {e}")
            return False
        ...
        for selector in publish_selectors:
            try:
                ...
                if await btn.count() > 0 and await btn.is_visible(timeout=3000):
                    await btn.click()
                    logger.info("✅ [豆瓣] 已点击发布按钮")
                    await asyncio.sleep(2)
                    return True
            except Exception as e:
                logger.debug(f"[豆瓣] 发布按钮 {selector} 失败: {e}")
                continue
    ...
    async def _wait_for_result(self, page: Page) -> Dict[str, Any]:
        ...
        logger.error(f"❌ [豆瓣] 发布超时（已等待120秒）: {page.url}")
...
registry.register("douban", DoubanPublisher("douban", DOUBAN_CONFIG))
