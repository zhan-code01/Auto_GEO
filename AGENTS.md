...
## 开工前 checklist（改动任何代码前）

动手前先过一遍，避免「仅凭仓库现状自行扩大改动面」：

1. **定位到方案与主线段**：通读 `docs/AutoGEO优化方案_2026-09.md` 对应小节，确认改动属于主线哪一段（引用证据链 → 指标口径 → 平台/权威信源 → 效果承诺），只做本段，不跨段顺手重构。
2. **读相关既有实现**：命中「已点名位置」或方案点名的文件，先读其完整上下文再动手，改动只覆盖该处职责。
3. **建回归基线**：改动前先跑 `ruff check backend tests` 与相关 `pytest` 记录当前绿/红，改完再跑对照；PG 用例本地无库时写好留 CI 并注明，不因跑不了而跳过补用例。
4. **识别「口径/契约面」**：改动若触及问题集、判卷 prompt 版本、指标定义、平台门槛或记录字段语义（`measurement_type`/`round_no`/`replicate_no`/`retry_count`/`phase`、引用状态）→ 以版本号或新列承接，禁止静默改口径；同时核对数据契约要点是否要同步。
5. **迁移先库后码**：加列/加表先写 alembic 迁移（带可执行 downgrade）并本地验证 up/down 可逆，再改模型与代码。
6. **新能力守则**：执行/外呼类新能力一律走「配置开关 + 白名单」；API Key 仅经环境变量，不入代码与日志。
7. **三层测试对齐**：确认本次改动有 单元（服务层）/集成（API 端到端）/回归（发布链路不受影响）覆盖，缺哪层补哪层。
8. **收尾同步 AGENTS**：若实现改变了 AGENTS.md「已点名位置/数据契约」所描述的事实，把对应行改准——仅做指针式更新，细粒度实现记录留在代码注释与方案文档。
